...
module.exports.run = async (client, message, args, Discord) => {
  ...
  const embed1 = new Discord.MessageEmbed()
    .setColor("RANDOM")
    .setThumbnail(
      `https://cdn.discordapp.com/attachments/730714810614022228/900375278734442496/310182.png`
    )
    .setAuthor(
      "Jarvis",
  ...
  switch (args[0]) {
        ...
        collector.on("collect", async (collected) => {
          if (collected.customId === "yes") {
            ...
            const embed7 = new Discord.MessageEmbed()
              ...
              .setDescription(
                `Type the song name u want to add to the playlist `
              )
    ...
    case "remove":
      if (!args[1]) {
        message.reply(
          `Pls mention song number from your playlist , to view your playlist type \`p!pl view\` \n Correct usage : Eg : \`p!pl remove 2\``
        );
        return;
      }
      if (isNaN(args[1]) || data.playlist.length < args[1]) {
        ...
        const embed9 = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Removed Song number \`${args[1]}\` from your playlist`
          )
    ...
    case "view":
      if (data && data.playlist.length > 0) {
        ...
        const chunked = chunkArray(data.playlist, 10);
        let currentPage = 0;
        ...
        const sendEmbed = async () => {
          const embed3 = new Discord.MessageEmbed()
            ...
            .setDescription(
              chunked[currentPage]
                .map((song, id) => ` **${id + 1 + currentPage * 10}**.   ${song.song} `)
                .join("\n")
            )
            .setFooter(
              `Page ${currentPage + 1}/${chunked.length} | Add songs p!pl add`,
              message.client.user.displayAvatarURL()
            )
            .setTimestamp();

          try {
            if (!message.lastEmbed) {
              // Send a new message for the first time
              const sentMessage = await message.channel.send({
                embeds: [embed3],
              });
              message.lastEmbed = sentMessage;
            } else {
              // Edit the existing message
              await message.lastEmbed.edit({ embeds: [embed3] });
            }
            if (chunked.length > 1) {
                await message.lastEmbed.react('⬅️');
                await message.lastEmbed.react('➡️');

                const filter = (reaction, user) => ['⬅️', '➡️'].includes(reaction.emoji.name) && user.id === message.author.id;
                const collector = message.lastEmbed.createReactionCollector({ filter, time: 60000 });
                ...
                collector.on('collect', (reaction) => {
                    ...
                    if (reaction.emoji.name === '⬅️' && currentPage > 0) {
                        currentPage--;
                    } else if (reaction.emoji.name === '➡️' && currentPage < chunked.length - 1) {
                        currentPage++;
                    }
                ...
                collector.on('end', () => {
                    message.lastEmbed.reactions.removeAll().catch(() => {});
                });
            }
          } catch (err) {
            console.log(err);
        ...
        const embed4 = new Discord.MessageEmbed()
          .setColor("RANDOM")
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/730714810614022228/900356283146829834/maxresdefault.png"
          )
          .setAuthor(
            "Jarvis",
            "https://cdn.discordapp.com/avatars/778267007439077396/66fa9525d6e9af153dac819fc04d3ee1.webp"
    ...
    case "play": //message.reply("Sorry, this feature is currently unavailable");
      if (!args[1]) {
        message.reply(
          `Please mention song number u want to play from your playlist \n \`Eg: p!pl play 2\` `
        );
      }
      if (isNaN(args[1] || data.playlist.length < args[1])) {
        message.reply("Invalid song number");
        ...
        if (times == 1) {
          data.playlist.map((song, id, playlist) => {
            if (times == 1) {
              ...
              playMusic(client, message, music).catch((error) => {
          ...
          playlist.findOneAndDelete(
            ...
            (err, res) => {
              ...
              const embed8 = new Discord.MessageEmbed()
                .setColor("RANDOM")
                .setThumbnail(
                  "https://media.discordapp.net/attachments/730714810614022228/900375278734442496/310182.png?width=885&height=498"
                )
                .setAuthor(
                  "Jarvis",
                  "https://cdn.discordapp.com/avatars/778267007439077396/66fa9525d6e9af153dac819fc04d3ee1.webp"
