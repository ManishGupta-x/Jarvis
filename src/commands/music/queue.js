	...
	if (!queue) {
		...
		const status = (queue) =>
			`Volume: \`${queue.volume}%\` | Loop: \`${
				queue.repeatMode ? (queue.repeatMode == 2 ? "Repeat queue" : "Repeat song") : "Off"
			}\` | Autoplay: \`${queue.autoplay ? "On" : "Off"}\``;

		let y = Util.splitMessage(
			`__**Current Queue**__\n\n${queue.songs
		...
		collector.on("collect", async (collected) => {
			...
			switch (value) {
				...
				case "first5":
					...
					break;
				case "second2":
					await client.distube.jump(message, 1);
					break;
				case "second3":
					await client.distube.jump(message, 1);
					break;
				...
				case "second5":
					...
					break;
				case "third3":
					await client.distube.jump(message, 2);
					break;
				case "third4":
					await client.distube.jump(message, 2);
					break;
				case "third5":
					...
					break;
				case "fourth4":
					await client.distube.jump(message, 3);
					break;
				case "fourth5":
					await client.distube.jump(message, 3);
					break;
				case "fifth5":
					await client.distube.jump(message, 4);
					break;
			}
